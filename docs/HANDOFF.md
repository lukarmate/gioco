...
- ✅ **Pool obiettivi: 22/22 completo.** Tutti i meccanismi: snapshot, accumulatori per-turno, streak. Contatori: kill-attribution (`CreatureNemicheDistrutte`+QuestoTurno via eventi `CreaturaDistrutta`), `DannoSubitoFinestra` (reset a fine turno), `CreatureColpisconoFaccia` (distinti), `LeaderHaColpitoFaccia`, `HeroPowerTurniUsati`, attacchi/energia-spesa per-turno. `Obiettivi.Pool` espone i 22.
...
| Leader | Leader: Zona Comando, hero power, passiva, morte→ritorno + costo incrementale | ✅ `StatoLeader` + `GiocaLeader` (costo base + `IncrementoLeader`*morti) · morte→Zona Comando · `AttivaHeroPower` (`DefCarta.HeroPower`, costo+cooldown) · passiva attiva da Zona Comando |
